...
import numpy as np
from my_utils import subsequent_mask, seq_padding
import csv
import torch
...
class PrepareData:
    def __init__(self, train_file, dev_file):
        ...
        # 构建单词表
        self.en_word_dict, en_total_words, self.en_index_dict = self.build_dict(self.train_en)
        self.cn_word_dict, cn_total_words, self.cn_index_dict = self.build_dict(self.train_cn)
    ...
    def save_to_file(self):
        """
        保存en_word_dict, cn_word_dict, en_index_dict, cn_index_dict
        """
        ...
        for i, data in enumerate(data_list):
            with open('data/word_name_dict_my/' + file_name_list[i] + '.csv', 'a+', encoding='utf-8',
                      newline='') as f:
                writer = csv.writer(f)
